...
public class DodgeAnimator extends Animator {
    ...
    public void onCameraSetUp(float tickDelta, Camera camera, Player clientPlayer, Parkourability parkourability) {
        ...
        float phase = (float) ((getTick() + tickDelta) / Dodge.MAX_TICK);
        ...
        switch (direction) {
            case Front: {
                float bodyPitchFactor = new Easing(phase)
                        .sinInOut(0, 1, 0, 1)
                        .get();
                access.parcool$setPitch(access.parcool$getXRot() + 360 * bodyPitchFactor);
                break;
            }
            case Left:
            case Right: {
                ...
                if (direction == Dodge.DodgeDirection.Right)
                    access.parcool$setRoll(access.parcool$getRoll() + 360 * rollFactor);
                else
                    access.parcool$setRoll(access.parcool$getRoll() - 360 * rollFactor);
                break;
            }
