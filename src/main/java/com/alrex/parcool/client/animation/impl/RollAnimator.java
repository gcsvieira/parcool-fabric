...
public class RollAnimator extends Animator {
    ...
    public void onCameraSetUp(float tickDelta, Camera camera, Player clientPlayer, Parkourability parkourability) {
        ...
        switch (direction) {
            case Front:
            case Back: {
                onCameraSetUpFrontBack(tickDelta, access, clientPlayer, parkourability);
            }
            break;
            case Left:
            case Right: {
                onCameraSetUpLeftRight(tickDelta, access, clientPlayer, parkourability);
            }
            break;
        }
    }

    void onCameraSetUpFrontBack(float tickDelta, CameraAccess camera, Player clientPlayer, Parkourability parkourability) {
        Roll roll = parkourability.get(Roll.class);
        float sign = direction == Roll.Direction.Front ? 1 : -1;
        if (roll.isDoing() &&
            ...
            float factor = calculateMovementFactor((float) ((roll.getDoingTick() + tickDelta) / (float) roll.getRollMaxTick()));
            camera.parcool$setPitch(sign * (factor > 0.5 ? factor - 1 : factor) * 360f + clientPlayer.getViewXRot(tickDelta));
        }
    }

    void onCameraSetUpLeftRight(float tickDelta, CameraAccess camera, Player clientPlayer, Parkourability parkourability) {
        float phase = (float) ((getTick() + tickDelta) / parkourability.get(Roll.class).getRollMaxTick());
        if (phase > 1) {
            return;
        }
        if (parkourability.get(Roll.class).isDoing() &&
            ...
            if (direction == Roll.Direction.Right)
                camera.parcool$setRoll(camera.parcool$getRoll() + 360 * rollFactor);
            else
                camera.parcool$setRoll(camera.parcool$getRoll() - 360 * rollFactor);
        }
    }
