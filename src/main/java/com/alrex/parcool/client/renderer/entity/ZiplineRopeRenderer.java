...
public class ZiplineRopeRenderer extends EntityRenderer<ZiplineRopeEntity, ZiplineRopeRenderState> {
    ...
    @Override
    public boolean shouldRender(@NotNull ZiplineRopeEntity entity, @NotNull Frustum frustum, double x, double y, double z) {
        return entity.shouldRender(x, y, z);
    }

    @Override
    public @NotNull ZiplineRopeRenderState createRenderState() {
    ...
    public void extractRenderState(@NotNull ZiplineRopeEntity entity, @NotNull ZiplineRopeRenderState state, float partialTick) {
        ...
        state.startBlockLightLevel = entity.level().getBrightness(LightLayer.BLOCK, state.startPos);
        state.endBlockLightLevel = entity.level().getBrightness(LightLayer.BLOCK, state.endPos);
    ...
    @Override
    public void submit(@NotNull ZiplineRopeRenderState renderState, @NotNull PoseStack poseStack, @NotNull SubmitNodeCollector nodeCollector, @NotNull CameraRenderState cameraRenderState) {
        boolean render3d = ParCoolConfig.Client.Booleans.Enable3DRenderingForZipline.get();
        var ropeRenderType = render3d ? ModRenderTypes.ZIPLINE_3D : ModRenderTypes.ZIPLINE_2D;
        nodeCollector.submitCustomGeometry(poseStack, ropeRenderType, (SubmitNodeCollector.CustomGeometryRenderer) new RopeRenderer(renderState));
    }
    ...
    private static class RopeRenderer implements SubmitNodeCollector.CustomGeometryRenderer {
        ...
        @Override
        public void render(@NotNull PoseStack.Pose pose, @NotNull VertexConsumer vertexConsumer) {
