...
public class Flipping extends Action {
    ...
    public boolean canStart(Player player, Parkourability parkourability, ByteBuffer startInfo) {
        ...
        ParCoolConfig.Client.FlippingControl control = ParCoolConfig.Client.getInstance().flipControl;
        startInfo
                .putInt(control.ordinal())
                .putInt(fDirection.ordinal());
        
        // Use the internal enum for logic
        ControlType logicControl = ControlType.values()[control.ordinal()];
        boolean input = logicControl.isInputDone(justJumped);
        justJumped = false;
        return (input
                && !player.isInWater()
