...
public class HangDown extends Action {
    ...
    public boolean canStart(Player player, Parkourability parkourability, ByteBuffer startInfo) {
        ...
        return (Math.abs(player.getDeltaMovement().y) < 0.2
                ...
                && (KeyBindings.getKeyHangDown().equals(KeyBindings.getKeySneak()) || !player.isShiftKeyDown())
        );
    }

    @Override
    public boolean canContinue(Player player, Parkourability parkourability) {
        return (!((ParkourabilityAccess) player).parcool$getStamina().isExhausted()
                && KeyBindings.getKeyHangDown().isDown()
    ...
    public void onStartInLocalClient(Player player, Parkourability parkourability, ByteBuffer startData) {
        ...
        if (!KeyBindings.getKeyHangDown().equals(KeyBindings.getKeySneak())) {
            parkourability.getBehaviorEnforcer().addMarkerCancellingSneak(ID_SNEAK_CANCEL, this::isDoing);
        }
        if (ParCoolConfig.Client.Booleans.EnableActionSounds.get()) {
            player.playSound(SoundEvents.HANG_DOWN.value(), 1.0f, 1.0f);
        }
    }
    ...
    public void onRenderTick(Object event, Player player, Parkourability parkourability) {
        if (isDoing()) {
            ...
            double differenceAngle = Math.acos(Math.max(-1, Math.min(1, bodyVec.dot(idealLookVec))));
            differenceAngle /= 4;
