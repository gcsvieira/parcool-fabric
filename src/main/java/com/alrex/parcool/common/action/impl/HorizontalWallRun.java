...
public class HorizontalWallRun extends Action {
    ...
    private int getMaxRunningTick(ActionInfo info) {
        return Math.min(
                ParCoolConfig.Client.Integers.WallRunContinuableTick.get(),
                ParCoolConfig.Server.Integers.MaxWallRunContinuableTick.get()
        );
    }
    ...
    public void onWorkingTickInLocalClient(Player player, Parkourability parkourability) {
        ...
        float slipperiness = player.level().getBlockState(leanedBlock).getBlock().getFriction();
        if (slipperiness <= 0.8) {
    ...
    public boolean canStart(Player player, Parkourability parkourability, ByteBuffer startInfo) {
        ...
        BufferUtil.wrap(startInfo).putBoolean(dividedVec.z() > 0);
        ...
        return (!parkourability.get(WallJump.class).justJumped()
                && (
                (ParCoolConfig.Client.getInstance().hWallRunControl.get() == ParCoolConfig.Client.HorizontalWallRunControl.PressKey && KeyBindings.getKeyHorizontalWallRun().isDown())
                        || ParCoolConfig.Client.getInstance().hWallRunControl.get() == ParCoolConfig.Client.HorizontalWallRunControl.Auto
        )
    ...
    public void onStartInLocalClient(Player player, Parkourability parkourability, ByteBuffer startData) {
        ...
        if (ParCoolConfig.Client.Booleans.EnableActionSounds.get())
            player.playSound(SoundEvents.HORIZONTAL_WALL_RUN.value(), 1f, 1f);
        ...
        if (animation != null) animation.setAnimator(new com.alrex.parcool.client.animation.impl.HorizontalWallRunAnimator(wallIsRightward));
    }
